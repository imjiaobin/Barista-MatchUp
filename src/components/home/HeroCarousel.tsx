  ...
  return (
    <section className="relative min-h-[calc(100vh-4rem)] bg-white flex flex-col justify-center overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto px-6 w-full grid md:grid-cols-2 items-stretch min-h-[calc(100vh-4rem)]">
        ...
        <div className="relative z-10 flex flex-col justify-center py-32 md:py-24 md:pr-16">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="flex flex-wrap gap-4">
                <Link href="/contact" className="btn-primary bg-caramel">立即諮詢</Link>
                <Link href="/events" className="btn-outline border-caramel text-caramel hover:bg-caramel">查看活動經歷</Link>
              </div>
            </motion.div>
          </AnimatePresence>
          ...
          <div className="flex items-center gap-4 mt-14">
              ...
              {slides.map((_, i) => (
                <button
                  ...
                  className={`h-0.5 rounded-full transition-all duration-300 ${
                    i === current ? 'w-8 bg-caramel' : 'w-4 bg-stone-200'
                  }`}
                />
              ))}
            </div>
            <div className="flex gap-1 ml-auto">
              <button
                onClick={prev}
                className="w-9 h-9 border border-stone-200 hover:border-caramel flex items-center justify-center transition-colors duration-200 text-stone-400 hover:text-caramel"
              >
        ...
        {/* 圖片／輪播側 —— 桌面版是 grid 欄位，手機版是絕對定位的背景 */}
